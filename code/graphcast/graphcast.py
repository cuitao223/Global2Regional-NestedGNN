  ...
  * Mesh2Grid graph: Graph that contains all nodes. This graph is strictly
  ...
  def _init_grid2mesh_graph(self) -> typed_graph.TypedGraph:
    ...
    grid_node_set = typed_graph.NodeSet(
        n_node=n_grid_node,
        features=_to_tensor(senders_node_features, device=self._device))
    mesh_node_set = typed_graph.NodeSet(
        n_node=n_mesh_node,
        features=_to_tensor(receivers_node_features, device=self._device))
    edge_set = typed_graph.EdgeSet(
        n_edge=n_edge,
        indices=typed_graph.EdgesIndices(senders=senders, receivers=receivers),
        features=_to_tensor(edge_features, device=self._device))
  ...
  def _init_mesh_graph(self) -> typed_graph.TypedGraph:
    ...
    mesh_node_set = typed_graph.NodeSet(
        n_node=n_mesh_node,
        features=_to_tensor(node_features, device=self._device))
    edge_set = typed_graph.EdgeSet(
        n_edge=n_edge,
        indices=typed_graph.EdgesIndices(senders=senders, receivers=receivers),
        features=_to_tensor(edge_features, device=self._device))
  ...
  def _run_grid2mesh_gnn(self, grid_node_features: torch.Tensor,
    ...
    structural_grid = grid_nodes.features.to(device=grid_node_features.device,
                                              dtype=grid_node_features.dtype)
    new_grid_nodes = grid_nodes._replace(
        features=torch.cat([
            grid_node_features,
            _add_batch_second_axis(structural_grid, batch_size)
        ], dim=-1))
    ...
    structural_mesh = mesh_nodes.features.to(device=grid_node_features.device,
                                             dtype=grid_node_features.dtype)
    new_mesh_nodes = mesh_nodes._replace(
        features=torch.cat([
            dummy_mesh_node_features,
            _add_batch_second_axis(structural_mesh, batch_size)
        ], dim=-1))

    grid2mesh_edges_key = grid2mesh_graph.edge_key_by_name("grid2mesh")
    edges = grid2mesh_graph.edges[grid2mesh_edges_key]
    structural_edges = edges.features.to(device=grid_node_features.device,
                                         dtype=grid_node_features.dtype)
    new_edges = edges._replace(
        features=_add_batch_second_axis(structural_edges, batch_size))
  ...
  def _run_mesh_gnn(self, latent_mesh_nodes: torch.Tensor) -> torch.Tensor:
    ...
    new_edges = edges._replace(
        features=_add_batch_second_axis(
            edges.features.to(device=latent_mesh_nodes.device,
                              dtype=latent_mesh_nodes.dtype), batch_size))
  ...
  def _run_mesh2grid_gnn(self,
                         updated_latent_mesh_nodes: torch.Tensor,
                         latent_grid_nodes: torch.Tensor,
                         ) -> torch.Tensor:
    ...
    new_edges = edges._replace(
        features=_add_batch_second_axis(
            edges.features.to(device=latent_grid_nodes.device,
                              dtype=latent_grid_nodes.dtype), batch_size))
  ...
  def _inputs_to_grid_node_features(
    ...
    grid_xarray_lat_lon_leading = model_utils.lat_lon_to_leading_axes(
        stacked_inputs)
    data = grid_xarray_lat_lon_leading.data
    if isinstance(data, torch.Tensor):
      tensor = data.to(device=self._device, dtype=torch.float32)
    else:
  ...
  def _grid_node_outputs_to_prediction(
    ...
    grid_outputs_lat_lon_leading = grid_node_outputs.detach().cpu().numpy().reshape(
  ...
  def forward_grid_node_outputs(
      self,
      inputs: xarray.Dataset,
      forcings: xarray.Dataset,
  ) -> torch.Tensor:
    """Returns raw grid-node outputs without converting to xarray."""
    self._maybe_init(inputs)
    grid_node_features = self._inputs_to_grid_node_features(inputs, forcings)
    latent_mesh_nodes, latent_grid_nodes = self._run_grid2mesh_gnn(
        grid_node_features)
    updated_latent_mesh_nodes = self._run_mesh_gnn(latent_mesh_nodes)
    return self._run_mesh2grid_gnn(updated_latent_mesh_nodes, latent_grid_nodes)

  def to(self, device) -> "GraphCast":
    device = torch.device(device)
    if device == self._device:
      return self
    self._device = device
    self._grid2mesh_gnn.to(device)
    self._mesh_gnn.to(device)
    self._mesh2grid_gnn.to(device)
    self._initialized = False
    self._grid2mesh_graph_structure = None
    self._mesh_graph_structure = None
    self._mesh2grid_graph_structure = None
  ...
  def reset_grid_cache(self) -> None:
    """Forces regeneration of cached grid-dependent structures on next call."""
    self._initialized = False
    self._grid2mesh_graph_structure = None
    self._mesh_graph_structure = None
    self._mesh2grid_graph_structure = None
  ...
  def parameters(self):
    for module in (self._grid2mesh_gnn, self._mesh_gnn, self._mesh2grid_gnn):
      for param in module.parameters():
        yield param

  def state_dict(self) -> dict[str, Any]:
    """Return state for serialization, mirroring torch.nn.Module semantics."""
    return {
        "grid2mesh_gnn": self._grid2mesh_gnn.state_dict(),
        "mesh_gnn": self._mesh_gnn.state_dict(),
        "mesh2grid_gnn": self._mesh2grid_gnn.state_dict(),
    }
  ...
  def load_state_dict(
    ...
    """Load serialized weights into the underlying Torch modules."""
    expected_modules = (
        ("grid2mesh_gnn", self._grid2mesh_gnn),
        ("mesh_gnn", self._mesh_gnn),
        ("mesh2grid_gnn", self._mesh2grid_gnn),
    )
    missing = []
    for key, module in expected_modules:
    ...
    if strict and missing:
      raise KeyError(f"Missing state dict entries for: {', '.join(missing)}")
    # Rebuild cached graph structures on next forward pass.
    self.reset_grid_cache()

  def load_haiku_parameters(
      self,
      npz_path: str,
      sample_inputs: xarray.Dataset,
      sample_targets_template: xarray.Dataset,
      sample_forcings: xarray.Dataset,
  ) -> None:
    """Loads parameters converted from the original JAX/Haiku implementation."""
    ...
    if original_device.type != "cpu":
      ...
      moved_to_cpu = True
    with torch.no_grad():
      self(sample_inputs, targets_template=sample_targets_template,
           forcings=sample_forcings)

    with np.load(npz_path) as data:
      params_dict = {k: data[k] for k in data.files if k.startswith("params:")}

    self._grid2mesh_gnn.load_haiku_parameters(
        params_dict, "params:grid2mesh_gnn")
    self._mesh_gnn.load_haiku_parameters(params_dict, "params:mesh_gnn")
    self._mesh2grid_gnn.load_haiku_parameters(
        params_dict, "params:mesh2grid_gnn")
...
def _add_batch_second_axis(data: torch.Tensor, batch_size: int) -> torch.Tensor:
  # data [leading_dim, trailing_dim]
  assert data.ndim == 2
  ones = torch.ones(batch_size, 1, dtype=data.dtype, device=data.device)
  return data.unsqueeze(1) * ones  # [leading_dim, batch, trailing_dim]
