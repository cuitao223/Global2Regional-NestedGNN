#!/usr/bin/env python3
"""Rolling evaluation for nested regional GraphCast forecasts (baseline)."""
...
from graphcast import data_pipeline, normalization, npz_utils
...
DEFAULT_REGIONAL_CKPT = DATA_DIR / "dataset" / "params" / "regional_graphcast.pt"
...
def _load_model_with_cache(
    ...
    if status == "from_npz":
        model.load_haiku_parameters(
            ...
            sample_targets_template=sample_targets,
...
def _apply_boundary_to_dataset(
    dataset: xr.Dataset,
    boundary_patch: xr.Dataset,
    target_index: int,
    lat_slice: slice,
    lon_slice: slice,
) -> None:
    time_coord = dataset.coords["time"][target_index]
    selectors = {"time": time_coord}
    if "batch" in dataset.dims:
        selectors["batch"] = dataset.coords["batch"][0]

    patch = boundary_patch.astype("float32")
    patch = patch.isel(batch=0)
    if "time" in patch.dims:
        patch = patch.isel(time=0)

    lat_values = dataset.lat[lat_slice]
    lon_values = dataset.lon[lon_slice]

    for name, data in patch.data_vars.items():
        if name not in dataset:
            continue
        dataset[name].loc[selectors | {"lat": lat_values, "lon": lon_values}] = data


def _extract_prediction_frame(prediction: xr.Dataset) -> xr.Dataset:
    frame = {}
    for name, data in prediction.data_vars.items():
        sample = data
        if "batch" in sample.dims:
            sample = sample.isel(batch=0)
        if "time" in sample.dims:
            sample = sample.isel(time=-1)
        frame[name] = sample
    return xr.Dataset(frame)
...
def build_metrics_dataframe(
    ...
    # global models (e.g. 0.25° → 1° via mean over 4×4 cells).
...
def plot_surface_step(
    ...
    # Use global (over time) ranges so different leads share the same
    # colour scale, making panels comparable.
    full_pred_t2m = pred["2m_temperature"]
    full_obs_t2m = obs["2m_temperature"]
    full_pred_mslp = pred["mean_sea_level_pressure"]
    full_obs_mslp = obs["mean_sea_level_pressure"]

    t_min = float(np.nanmin([full_pred_t2m.min().values, full_obs_t2m.min().values]))
    t_max = float(np.nanmax([full_pred_t2m.max().values, full_obs_t2m.max().values]))
    t_diff_abs = float(
        np.nanmax(np.abs((full_pred_t2m - full_obs_t2m).values))
    )

    p_min = float(np.nanmin([full_pred_mslp.min().values, full_obs_mslp.min().values]))
    p_max = float(np.nanmax([full_pred_mslp.max().values, full_obs_mslp.max().values]))
    p_diff_abs = float(
        np.nanmax(np.abs((full_pred_mslp - full_obs_mslp).values))
    )
    ...
    fields = [
        ...
        (pred_mslp, "p", "Regional MSLP (hPa)"),
        (obs_mslp, "p", "ERA5 MSLP (hPa)"),
        (diff_mslp, "p_diff", "ΔMSLP (hPa)"),
    ]

    meshes: dict[str, list] = {"t": [], "t_diff": [], "p": [], "p_diff": []}
    ...
    for idx, (ax, (field, kind, title)) in enumerate(zip(axes.ravel(), fields, strict=True)):
        ...
        if kind == "t":
            ...
            vmin, vmax = -t_diff_abs, t_diff_abs
            cmap = "RdBu_r"
        elif kind == "p":
            vmin, vmax = p_min, p_max
            cmap = "viridis"
        else:  # "p_diff"
            vmin, vmax = -p_diff_abs, p_diff_abs
            cmap = "RdBu_r"
    ...
    axes[0, 0].quiver(
        ...
        pred_u.values[::stride_lat, ::stride_lon],
        pred_v.values[::stride_lat, ::stride_lon],
        transform=proj,
        color="k",
    )
    axes[0, 1].quiver(
        lon.values[::stride_lon],
        lat.values[::stride_lat],
        obs_u.values[::stride_lat, ::stride_lon],
        obs_v.values[::stride_lat, ::stride_lon],
        transform=proj,
        color="k",
    )
    axes[0, 2].quiver(
        lon.values[::stride_lon],
        lat.values[::stride_lat],
        (pred_u - obs_u).values[::stride_lat, ::stride_lon],
        (pred_v - obs_v).values[::stride_lat, ::stride_lon],
        transform=proj,
        color="k",
    )
    ...
    if meshes["t"]:
        ...
        cbar_t.set_label("2m T (K)")
    if meshes["t_diff"]:
        cbar_td = fig.colorbar(
            meshes["t_diff"][0],
            ax=axes[0, 2],
            orientation="horizontal",
            pad=0.06,
    ...
    if meshes["p"]:
        ...
        cbar_p.set_label("MSLP (hPa)")
    if meshes["p_diff"]:
        cbar_pd = fig.colorbar(
            meshes["p_diff"][0],
            ax=axes[1, 2],
            orientation="horizontal",
            pad=0.10,
        ...
        cbar_pd.set_label("ΔMSLP (hPa)")

    fig.suptitle(f"Nested regional surface fields @ +{lead_hour} h", fontsize=13)
    fig.tight_layout(rect=(0.02, 0.02, 0.98, 0.94))

    path = out_dir / f"regional_surface_step_{lead_hour:03d}h.png"
    fig.savefig(path, dpi=200)
    plt.close(fig)
    return path
...
def plot_surface_step_pretty(
    ...
    """Improved visualisation with physically reasonable colour scales (no wind arrows)."""
    pred = squeeze_batch(predictions)
    obs = squeeze_batch(targets)
    ...
    # ---- Physically constrained colour ranges with simple outlier handling ----
    T_MIN_DEFAULT, T_MAX_DEFAULT = 250.0, 320.0  # K
    P_MIN_DEFAULT, P_MAX_DEFAULT = 98000.0, 103000.0  # Pa
    T_DIFF_DEFAULT = 5.0  # K
    P_DIFF_DEFAULT = 10.0  # hPa, fixed symmetric range for ΔMSLP
    ...
    # For ΔMSLP we intentionally use a fixed narrow range in hPa.
    ...
    for idx, (ax, (field, kind, title)) in enumerate(zip(axes.ravel(), fields, strict=True)):
        ...
        # Independent colourbar for each panel
        if kind == "t":
            label = "2m T (K)"
        elif kind == "t_diff":
            label = "Δ2m T (K)"
    ...
    fig.suptitle(f"Nested regional surface fields @ +{lead_hour} h", fontsize=13)
    # extra bottom margin for longitude labels + colourbars
...
def plot_wind_step(
    ...
    """Standalone 10 m wind vectors for baseline eval (pred, ERA5, difference)."""
    pred = squeeze_batch(predictions)
    obs = squeeze_batch(targets)
    lead_idx = int(np.where(to_hours(pred.time.values) == lead_hour)[0][0])
    ...
    fig.suptitle(f"Nested regional 10m wind @ +{lead_hour} h", fontsize=13)
...
def plot_precip_proxy_maps(
    ...
    for idx, lead in enumerate(leads):
        for row, (data, title, cmap, vlims) in enumerate(
            (
                (proxy_pred.isel(time=idx), f"Regional ω×q {level} hPa @ +{lead}h", "PuOr", (-vmax, vmax)),
    ...
    # 共享颜色条：每一行一个
    labels = [
        f"Regional proxy ω×q @ {level} hPa",
    ...
    fig.suptitle(f"Nested regional precip proxy (ω×q) @ {level} hPa", fontsize=13)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Nested GraphCast rolling evaluation.")
...
def run_nested_rollout(args: argparse.Namespace) -> EvalArtefacts:
    ...
    regional_predictor = normalization.InputsAndResiduals(
        regional_model,
        stats_std_reg,
        stats_mean_reg,
        diff_std_reg,
    ...
    for step_idx in range(total_steps):
        ...
        if args.verbose and step_idx == 0:
            sample_var = "mean_sea_level_pressure"
            if sample_var in pred_frame and sample_var in truth_frame:
                pred_sample = float(pred_frame[sample_var].mean().values)
                truth_sample = float(truth_frame[sample_var].mean().values)
                log(
                    f"[debug] mean({sample_var}) pred={pred_sample:.2f}, truth={truth_sample:.2f}",
                    enable=True,
                )
...
def main() -> None:
    ...
    artefacts = run_nested_rollout(args)
    ...
    log("Nested rolling evaluation completed.", enable=True)
